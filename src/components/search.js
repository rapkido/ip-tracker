...
class Search extends React.Component {
    ...
    ipInputSet(e) {
        this.setState({
            ip_input: e.target.value
        })
    }
    onSearch(e) {
        
        var input_value = this.state.ip_input;
        ...
        if (expression.test(input_value))
        {
            // good IP
            const {seek} = this.props;
            seek(this.state.ip_input);
        }
        else
        {
            // bad IP
            alert("Provide a valid IP adresss please!")
        }
    }
    handleKeypress(e) {       
        if (e.charCode === 13) {     
    ...
    render() {
        const {ip_input} = this.state;
        return(
            <div className="input" id="input-div">
                <input type="text" id="ip_in" name="ip_in" value={ip_input} onChange={this.ipInputSet.bind(this)} onKeyPress={this.handleKeypress.bind(this)} placeholder="Search for any IP address" />
                <button type="submit" onClick={this.onSearch.bind(this)} ><img src="assets/images/icon-arrow.svg" alt="" /></button>
            </div>
        );
    }
}
